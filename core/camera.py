...
class Camera:
    def __init__(self):
        ...
        self.intelbras = IntelbrasAPI(f"http://{self.camera_ip}").login(
            f"{self.camera_user}", f"{self.camera_password}"
        )
        
    
    def rtp(self):
        rtsp_url = f"rtsp://{self.camera_user}:{self.camera_password}@{self.camera_ip}:554/cam/realmonitor?channel=1&subtype=0"
